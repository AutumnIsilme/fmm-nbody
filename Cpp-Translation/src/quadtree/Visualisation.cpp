...
void draw_box_square(const Box &box, std::ostringstream &svg,
  ...
  if (box.has_child_boxes) {
    for (const auto &child : box.child_boxes) {
      if (child)
        draw_box_square(*child, svg, colour);
    }
  }
  write_box_outline(svg, box, colour);
}

void graph_box(const Box &box, std::ostringstream &svg,
               const std::string &box_colour,
               const std::string &particle_colour) {
  if (box.has_child_boxes) {
    for (const auto &child : box.child_boxes) {
      if (child)
        graph_box(*child, svg, box_colour, particle_colour);
    }
    return;
  }

  write_box_outline(svg, box, box_colour);
...
void graph_quadtree(const Box &root, const std::string &filepath,
  ...
  graph_box(root, svg, box_colour, particle_colour);
  svg << "</g>\n</svg>\n";
